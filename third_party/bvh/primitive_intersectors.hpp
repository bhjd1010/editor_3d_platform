...
struct PrimitiveIntersector {
    PrimitiveIntersector(const Bvh& bvh, const Primitive* primitives, const std::size_t inIgnoreIndex = -1)
		: bvh(bvh), primitives(primitives), ignoreIndex(inIgnoreIndex)
    {}
    ...
    const std::size_t ignoreIndex = -1;
...
struct AnyPrimitiveIntersector : public PrimitiveIntersector<Bvh, Primitive, Permuted, true> {
    ...
    AnyPrimitiveIntersector(const Bvh& bvh, const Primitive* primitives, const std::size_t inIgnoreIndex = -1)
		: PrimitiveIntersector<Bvh, Primitive, Permuted, true>(bvh, primitives, inIgnoreIndex)
    {}
